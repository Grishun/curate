...
Curate fetches cryptocurrency exchange rates for a target quote currency, stores history in InfluxDB 
(or in memory if configuired), and exposes them over an HTTP or gRPC streaming API
...
| `CURATE_GRPC_HOST`         | `--grpc-host`         | grpc server host                               | `127.0.0.1`                         |
| `CURATE_GRPC_PORT`         | `--grpc-port`         | grpc server port                               | `8081`                              |
...
go run ./cmd/service --in-memory-storage=true
```
...
- Upstream price provider. Additional providers can be added behind the same
  interface (check `internal/domain/interface.go`).
...
- Fiber server is exposing the OpenAPI contract, handlers, validation, request/response
  mapping.

### `internal/transport/grpc`

- Implementation of the gRPC server. Includes generated code from the proto file (generated by protoc), server
implementation, and gRPC handlers.

### `internal/clients/rest`

- shared HTTP client wrapper (resty) with logging for upstream calls.

### `internal/clients/grpc`

- implementation of gRPC client for streaming rates
